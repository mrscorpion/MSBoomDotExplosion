...
#import "MSSlimeDotView.h"

#define kMaxDistance 20

@interface MSSlimeView()

@property (nonatomic) CGFloat maxDistance;

@property (nonatomic, strong) MSSlimeDotView *headDot;
@property (nonatomic, strong) MSSlimeDotView *trailDot;
@property (nonatomic, strong) CAShapeLayer *shapLayer;

@end

@implementation MSSlimeView

- (instancetype) initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame: frame]) {
        self.maxDistance = kMaxDistance; //180;
    ...
    [UIView animateWithDuration: 0.25f
         ...
         usingSpringWithDamping: 0.5
          initialSpringVelocity: 0
                     ...
                     animations: ^{
                         self.headDot.center = self.center;
                         self.headDot.backgroundColor = MSThemeColor;
                     }
                     completion: ^(BOOL finished) {
...
#pragma mark - 懒加载
- (MSSlimeDotView *) headDot {
    if (!_headDot) {
        _headDot = [[MSSlimeDotView alloc] initWithFrame: CGRectMake(0, 0, HEADDOT_W, HEADDOT_W)];
    }
    return _headDot;
}

- (MSSlimeDotView *) trailDot {
    if (!_trailDot) {
        _trailDot = [[MSSlimeDotView alloc] initWithFrame: CGRectMake(0, 0, TRAILDOT_W_MAX, TRAILDOT_W_MAX)];
    }
    return _trailDot;
}
...
- (CAShapeLayer *) shapLayer {
    if (!_shapLayer) {
        ...
        _shapLayer.fillColor = MSThemeColor.CGColor;
